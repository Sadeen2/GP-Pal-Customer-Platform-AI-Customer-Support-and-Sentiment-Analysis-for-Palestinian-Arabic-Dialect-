...
from app.database.models import Message, Conversation, Customer, CustomerChannel, MessageAttachment
...
def get_or_create_customer_channel(db: Session, channel: str, external_id: str):
    """Find/create the real Customer and the channel identity row."""
    link = (
    ...
    log(f"created customer id={customer.id} channel={channel} external_id={external_id}")
    return customer, link
...
def save_bot_reply_message(
    ...
    bot_message = Message(
        ...
        matched_faq_id=bot_result.get("faq_id"),
