...
class User(Base):
    ...
    # Agent availability for automatic assignment.
    # Only users with role="agent", is_active=True, and is_available=True
    # are considered for automatic handoff/escalation assignment.
    is_available = Column(Boolean, default=True, nullable=False, index=True)
    max_active_conversations = Column(Integer, default=5, nullable=False)
...
class Admin(Base):
    """
    Extra admin profile table.
    Login credentials stay in users table, while admin-specific data lives here.
    One admin row belongs to one user whose role is super_admin or manager.
    """
...
class Conversation(Base):
    ...
    # When False, the bot stops auto-replying on this conversation so a
    # human agent can take over. Re-enabled manually when the agent is done.
